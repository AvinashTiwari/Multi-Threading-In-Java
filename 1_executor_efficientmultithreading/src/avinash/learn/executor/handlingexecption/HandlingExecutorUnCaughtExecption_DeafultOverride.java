...
public class HandlingExecutorUnCaughtExecption_DeafultOverride {
	public static void main(String[] args) {
		String currentThreadName = Thread.currentThread().getName();
		System.out.println("[ " + currentThreadName + " ] Main Thread starts Here");
		Thread.setDefaultUncaughtExceptionHandler(new ThreadExceptionHandler("Default_handler"));
		
		ExecutorService exServcie1 = Executors.newCachedThreadPool(new ThreadFactoryWithExceptionAlternator());
		
		exServcie1.execute(new ExecptionLeakingTask());
		exServcie1.execute(new ExecptionLeakingTask());
		exServcie1.execute(new ExecptionLeakingTask());
		
		ExecutorService exServcie2 = Executors.newCachedThreadPool(new ThreadFactoryWithExceptionAlternator());
